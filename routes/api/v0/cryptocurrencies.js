...
router.get("/:walletAddress", function (req, res) {
    let addressWallet = req.params.walletAddress;
    Promise.all([
        axios.get(
            `https://api.etherscan.io/api?module=account&action=balance&address=${addressWallet}&tag=latest&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.USDT}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.USDC}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.LINK}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.LRC}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.MKR}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.OMG}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.REPv2}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.SHIB}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.WBTC}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.ZRX}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
        axios.get(
            `https://api.etherscan.io/api?module=account&action=tokentx&contractaddress=${symbolToAddress.BAT}&address=${addressWallet}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${apiKeyToken}`
        ),
    ])
        .then((response) => {
            let balancesCrypto = [];
            ...
            const resposta = response.map((item, index) => {
                if (
                    item.data.message == "OK" &&
                    typeof item.data.result == "string"
                ) {
                    amountCrypto = parseTransactionValue(
                        parseInt(item.data.result),
                        18
                    );
                    priceCrypto = Math.random();
                    balancesCrypto.push({
                    ...
                    item.data.result.forEach((transaction) => {
                        if (
                            ...
                            transaction.from.toUpperCase() !=
                                addressWallet.toUpperCase() &&
                            transaction.to.toUpperCase() ==
                                addressWallet.toUpperCase()
                        ) {
                            walletBalance =
                                walletBalance + parseInt(transaction.value);
                        } else if (
                            transaction.to &&
                            transaction.to.toUpperCase() !=
                                addressWallet.toUpperCase() &&
...
module.exports = router;
>>>>>>> 516addc (Primeiro versão Produção)
